...
Clear-Host
Write-Host "`n  ╔═══════════════════════════════════════════════════════╗" -F Cyan
Write-Host "  ║      OGD WinCaffè v8.0.2HF4 - DISCLAIMER & CREDITI      ║" -F Cyan
Write-Host "  ╚═══════════════════════════════════════════════════════╝`n" -F Cyan
...
function Get-OgdCurrentScriptVersion {
    ...
    try{
        if($PSCommandPath){
            ...
            if($leaf -match '^OGD_WinCaffe_(?<ver>\d+\.\d+\.\d+(?:(?:HF|HotFix)\d+)?)\.ps1$'){
                return $Matches['ver']
            }
        }
    }catch{}
...
$currentVersion = Get-OgdCurrentScriptVersion -FallbackVersion '8.0.2HF4'
...
Clear-Host
Write-Host "`n  ╔═══════════════════════════════════════════════════════╗" -F Cyan
Write-Host "  ║           OGD WinCaffè v8.0.2HF4 - Installazione        ║" -F Cyan
Write-Host "  ╚═══════════════════════════════════════════════════════╝`n" -F Cyan
...
function Show-Banner {
    ...
    Write-Host "  ║     ▄▄▄▄▄▄▄  ▄▄▄▄▄▄▄  ▄▄▄▄▄▄    ⚡                  ║" -F Yellow
    Write-Host "  ║    █       ██       ██      █                        ║" -F Yellow
Write-Host "  ║    █   ▄   ██   ▄▄▄▄█  ▄    █     WinCaffè v8.0.2HF4 ║" -F Yellow
...
function Show-Banner {
    ...
    Write-Host ""
    Write-Host "  +--------------------------------------------------------------------+" -F Gray
    Write-Host "  | OGD WINCAFFE 8.0.2HF4 // CYBER GAMING WORKBENCH                    |" -F Black -BackgroundColor Gray
    Write-Host "  +--------------------------------------------------------------------+" -F Gray
...
function Show-OgdMainMenu801 {
    ...
    Write-Host "  | [H] HOTFIX 8.0.2HF4               [J] LEGACY FIX     [0] ESCI      |" -F Yellow
    Write-Host "  |                                                                    |" -F White
...
function Show-OgdAppliedSummary {
    ...
    switch($Profile){
        ...
        'FIXPRE778' {
Show-OgdWhatThisDoes 'Fix legacy: ramo storico dismesso in 8.0.2HF4' @(
...
function New-OgdOpenDyslexicRestorePoint {
    $desc = "OGD WinCaffe v8.0.2HF3 OpenDyslexic - $(Get-Date -Format 'dd/MM/yyyy HH:mm:ss')"
...
function Invoke-OgdPre798NetworkDiscordRepair {
    ...
    if($CreateRestorePoint){
        $desc798 = "OGD WinCaffè v8.0.2HF4 FIX RETE LEGACY - $(Get-Date -Format 'dd/MM/yyyy HH:mm')"
...
# Punto ripristino
$desc="OGD WinCaffè v8.0.2HF4 - $(Get-Date -Format 'dd/MM/yyyy HH:mm')"
New-OgdRestorePoint -Description $desc
...
Write-Host "  [H] 🩹 HOTFIX 8.0.2HF4 - DX9 legacy + OpenDyslexic + NPU diag" -F Cyan
...
Write-Host "  2. Seleziona: 'OGD WinCaffè v8.0.2HF4'`n" -F White
    Read-Host "  INVIO";continue MenuLoop
}
...
'@|Out-File $timerDest2 -Encoding UTF8 -Force
    ...
    Write-Host "`n  ════════════════════════════════════════════════════" -F Cyan
Write-Host "   OGD WinCaffè v8.0.2HF4" -F Yellow
    Write-Host "   #DarkPlayer84Tv Productions" -F Green
    Write-Host "   by OldGamerDarthy Official" -F Green
    Write-Host "  ════════════════════════════════════════════════════`n" -F Cyan
